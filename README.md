[![tests](https://github.com/qa-schema/qa-learning/actions/workflows/tests.yml/badge.svg)](https://github.com/qa-schema/qa-learning/actions/workflows/tests.yml)

# QA Learning Repository

This repository documents my journey to become a QA Engineer.

## About me

I am a Junior QA Engineer in training, building practical experience in software testing.

My goal is to grow from manual testing to test automation using Python and Selenium.

This repository is my personal learning portfolio where I:
- practice writing test cases and bug reports
- document my QA learning process
- build small automation scripts
- demonstrate my skills to potential employers

Current focus:
- Manual testing fundamentals
- Test case design
- Bug reporting
- Basic test automation (Python + Selenium)

Future goals:
- Advanced automation testing
- CI/CD basics
- Real-world project testing

## Skills snapshot
- Manual Testing (Test cases, Bug reports)
- Test design techniques (EP, BVA - basic)
- Selenium WebDriver (Python)
- (Git & GitHub)


## How to run automation tests
This section explains how to run the existing Selenium automation tests locally.

### Requirements
- Python 3.10 or newer
- Google Chrome browser
- ChromeDriver (mathing your Chrome version)

### Setup
1. Clone the repository
2. Install dependencies:
   pip install -r requirements.txt
### Run test
python automation/test_invalid_password_shows_error.py

## Structure

- test-cases - Manual and automated test cases
- bug-reports - Bug reports with reproduction steps
- automation - Automation scripts and experiments
- notes - Learning notes and theory
## Goals

- Improve QA skills
- Practice real test documentation
- Build a professional QA portfolio

## Tools

- Manual Testing
- Selenium / Python 
- GitHub


